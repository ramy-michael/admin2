...
// if($('#expense_type').val()==2){
  ...
  $('#client').val("{{$Transaction[0]->client_id}}");
  console.log("{{$Transaction[0]->client_id}}");
...
@endforeach
$('#date').val("{{$Transaction[0]->created_at}}")
  $('#balance').val("{{$Transaction[0]->depit+$Transaction[0]->account_balance}}");
  var old_depit="{{$Transaction[0]->depit}}";

  $('#amount').val("{{$Transaction[0]->depit}}");

    var remain =Number($('#balance').val())-Number($('#amount').val())
    $('#remain').val("{{$Transaction[0]->account_balance}}")

// var old_credit="{{$Transaction[0]->depit}}";
console.log('{{$Transaction[0]->account_balance}}')
  // $('#supplier').val("{{$Transaction[0]->supplier_id}}");


  // $("#supplier").select2("val", "{{$Transaction[0]->supplier_id}}"); //set the value
console.log($('#client').val(),{{$Transaction[0]->supplier_id}})
